...
public class LarkEventMessageRequestHandler : IRequestHandler<LarkEventMessage,LarkEventMessageResonseBase>
{
    ...
    public async Task<LarkEventMessageResonseBase> Handle(LarkEventMessage request, CancellationToken cancellationToken)
    {
        var encryptKey = await _daprClient.GetSecretAsync("local-secret-store", "LarkEventSubscriptionEncryptKey");
    ...
    public async Task<LarkEventMessageResonseBase> ResolveEventMessage(string eventMessage)
    {
        ...
        {
            return new UrlVerificationResponse { Challenge = root.GetProperty("challenge").GetString() };
        }
